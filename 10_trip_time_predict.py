...
for ind in xrange(len(df)):
    dist.append(haversine((df.pickup_latitude[ind],df.pickup_longitude[ind]),(df.dropoff_latitude[ind],df.dropoff_longitude[ind]),miles=1))
df['geo_dist'] = dist
df = pd.get_dummies(df,columns = ['vendor_id','rate_code','payment_type','pickup_dow','pickup_hour','pickup_month'])
df.drop(['fare_amount','tip_amount','total_amount','pickup_datetime','pickup_dt','dropoff_datetime','hack_license','medallion','trip_distance'],axis = 1,inplace = 1)
df.dropna(inplace = 1)
...
y = df.pop('trip_time_in_secs').values
x = df.values
...
explode = [0]*len(fracs)
explode[-1] = 0.1
labels = ['Geo Dist','Dropoff Lat','Dropoff Lon','Pickup Lon','Pickup Lat','Surcharge',
'Features Left Off']
colors = ['royalblue','g','m','c','violet','peachpuff','darkgray']
